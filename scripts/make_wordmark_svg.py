"""
Render "IamAyush" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it
as an SVG that animates on GitHub (SMIL only -- GitHub runs SVG animations in
<img>, but never JS).

Customized for: Ayush Kashyap (github.com/Ayushtechera)
Font swapped to Poppins Bold (Futura isn't available outside macOS; Poppins
is the closest freely-available geometric bold sans and gives the same clean
extrusion edges).
...
COLS = int(os.environ.get("WORDMARK_COLS", 90))  # "IamAyush" is longer than "AVI" -- wider grid needed
ROWS = 0
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 5))
CELL_W = 9.0
CELL_H = 15.5
# Poppins Bold: freely available, geometric-sans, even stroke weight -- closest
# stand-in for Futura Bold on a Linux box (no .ttc font-collection index needed).
FONT_PATH = os.environ.get(
    "WORDMARK_FONT",
    "/usr/share/fonts/truetype/google-fonts/Poppins-Bold.ttf",
)
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 0))
TEXT = os.environ.get("WORDMARK_TEXT", "IamAyush")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">ayush@github: ~$ ./wordmark.sh --3d</text>')
